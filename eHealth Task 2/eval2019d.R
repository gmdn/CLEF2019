# subset experiments
res_orig_p50 <- results %>% filter(grepl(x = run, pattern = "original_m10p50"))

topics <- sort(unique(qrels_abs_test$topic))

results_orig_p50 <- res_orig_p50 %>%
  group_by(run, topic) %>%
...
last_shown <- res_orig_p50 %>%
  filter(Q0 == 1) %>% 
  select(run, topic, rank)


for (run in unique(res_orig_p50$run)) { # run <- unique(res_orig_p50$run)[1]
  print(run)
  for(topic in topics) { # topic <- topics[1]
    ...
    if (nrow(last_r) == 0) {
      #print(topic)
      results_orig_p50 <- results_orig_p50 %>% 
...
levels <- c("2018_stem_original_m10p50f0t100p1m10s10",
            "2018_stem_original_m10p50f0t200p1m10s10",
            "2018_stem_original_m10p50f0t300p1m10s10",
            "2018_stem_original_m10p50f0t400p1m10s10",
            "2018_stem_original_m10p50f0t500p1m10s10",
            "2018_stem_original_m10p50f0t1000p1m10s10",
            "2018_stem_original_m10p50f0t1500p1m10s10")
